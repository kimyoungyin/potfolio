        ...
        challenges: [
            {
                ...
                caseStudy: {
                    problem: [
                        ...
                        "결국 이 문제는 인증 책임이 클라이언트에 과도하게 집중된 구조에서 비롯된 것으로, 분산된 진입점에서 동일한 인증 이벤트가 중복 처리되는 것이 핵심 병목이었습니다.",
                    ],
                    problemCode: {
                        caption:
...
return handleUnauthorizedAndRetry(originalRequest);`,
                    ...
                    investigation: [
                        ...
                        "하지만 브라우저가 인증 API를 직접 호출하는 구조로 인해 쿠키 전송과 CORS 문제는 남아 있었고, 이를 구조적 한계로 판단했습니다.",
                        "이 과정에서 인증 책임을 클라이언트에서 완전히 해결하는 것은 한계가 있다고 보고, 인증 흐름의 일부를 서버 경계로 이동시키는 방향으로 설계를 전환했습니다.",
                        "이에 따라 인증 요청을 same-origin으로 수렴시키는 BFF 패턴을 도입했습니다. 브라우저는 `/api/auth/*`만 호출하고, 서버가 백엔드를 호출하도록 인증 경계를 재획정했습니다.",
                        "또한 middleware에서 쿠키 존재 여부를 기준으로 접근 제어를 1차 분기하고, 인증 여부 확정은 클라이언트에서 보완하도록 역할을 분리했습니다.",
                        "마지막으로 서버 간 요청의 신뢰 경계를 보완하기 위해 내부 경유 요청을 식별하는 별도의 검증 계층을 두기로 결정했습니다.",
                    ...
                    impact: {
                        metrics: [
                            ...
                            },
                            {
                                ...
                                value: "깜빡임 0회",
                                description: [
                                    "미인증 상태로 보호 경로 접근 시 직접 확인 — 클라이언트 렌더 후 후처리에서 middleware 즉시 리다이렉트로 전환",
                                ],
                            },
                            {
                                label: "인증 API 직접 호출",
                                value: "0건",
                                description: [
                                    "브라우저 → 외부 인증 도메인 직접 호출 제거 — 쿠키 발급·CORS 설정 부담이 서버 경계 안으로 흡수됨",
                                ],
                            },
                        ],
                        summary: [
...
const total = data?.pages[0]?.total ?? 0;`,
                    ...
                    impact: {
                        ...
                        summary: [
                            "Supabase API 로그로 동일 게시글 재방문 시 캐시 히트 후 DB 쿼리가 발생하지 않음을 확인했습니다.",
                            "목록·검색의 SSR hydration 계약을 단일 방식으로 통일해 초기 렌더 단계의 클라이언트 중복 요청이 제거됐고, polling 제거로 백그라운드 불필요 요청도 없어졌습니다.",
                            "무효화 단위를 데이터 종류별 태그로 관리해 mutation 종류에 따라 영향받는 캐시만 갱신할 수 있는 구조가 됐습니다.",
                        ],
                    },
                },
            },
  ...
  getBookData();
                    ...
                    impact: {
                        metrics: [
                            ...
                            },
                            {
                                ...
                                description: [
                                    ...
                                    "Warm 캐시 위주로만 빠르던 구조에서, Cold Start 기준으로도 동일한 최적화 효과 확인",
                                ],
                            },
                            {
                        ...
                        summary: [
                            ...
                            "3단계에서는 데이터 흐름 자체를 바꿨습니다. 모달이 마운트된 뒤 useEffect에서 Server Action으로 DB를 조회하는 순차 워터폴을 `?loc=N` URL 파라미터로 대체해, location이 마운트 시점에 이미 확정되도록 했습니다. preload로 이미지가 캐시에 올라와 있는 상태와 맞물려 전환 직후 skeleton 없이 즉시 이미지가 표시되는 것을 브라우저 측정으로 확인했습니다.",
                        ],
                    },
                },
            },
